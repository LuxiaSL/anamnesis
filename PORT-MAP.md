...
| `anamnesis/extraction/feature_families/binding_probe.py` | `anamnesis/extraction/feature_families/binding_probe.py` | Admitted with the analysis port, which is where the dedupe and the test its admission was conditional on belonged. The row that records the admission, and what it rests on, is under **Admitted under ruling 7**. |
...
| `anamnesis/extraction/streaming_generate.py` | `anamnesis/extraction/streaming_generate.py` | The generation loop is logic-identical. The donor also carried a second calibration pass, with its own decode defaults and its own positional accumulation; it does not port. Calibration has one home, `anamnesis/extraction/calibration_fit.py`. |
...
| — | `tests/test_streaming_generate.py` | New. The loop's alignment contract, its determinism under a seed, end-of-sequence handling, and that hooks registered on the modules fire through it — which is what lets it replace the framework's loop without touching the capture surface. |
...
| `complementarity.BASELINE_TIER_ORDER` / `COMPOSITE_TIERS` / `BASELINE_BLOCK_BY_SIGNAL` / `analyze_tier_ordering` | `CORE_BLOCK_ORDER` / `BLOCK_UNION_LABELS` / `analyze_block_ordering`; the signal-to-block table does not port — one classifier answers that question, `anamnesis.feature_map.named_family`, and `complementarity` reads it. |
