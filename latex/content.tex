...
The experiment tested three generations of mode prompts, each addressing weaknesses identified in the prior.\footnote{The repository directories use a different numbering convention: paper Run 1 = \texttt{run2\_epistemic\_modes}, Run 2 = \texttt{run3\_process\_modes}, Run 3 = \texttt{run4\_format\_controlled}. The repository's \texttt{run1} was a failed initial attempt discarded due to pipeline bugs; it produced no usable data and is not reported here.} All modes were delivered as system prompts; user prompts followed a fixed template: ``Write about: \{topic\}.''
...
\textbf{Sensitivity to analogical mode.} Excluding analogical mode (which had distinctive KV cache behavior at 100\% accuracy), the 4-way tier ranking shifts: T2 (56.2\%) \textgreater{} T2.5 (47.5\%) \textgreater{} T1 (48.8\%). Analogical's unique lookback patterns and epoch structure drive T2.5's 5-way dominance.
...
\caption{Double dissociation: temperature variation activates T1 (90\%) with T2.5 at chance (35\%); processing mode activates T2.5 with T1 near chance. Values shown are kNN accuracy from the contrastive projection analysis (\S4.8); the table above reports RF accuracy on the same data. Both classifiers confirm the dissociation pattern.}
