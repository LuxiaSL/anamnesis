...
def test_one_transfer_pass_runs_end_to_end_and_carries_its_comparison(tmp_path: Path) -> None:
    ...
    assert set(forward["per_mode_accuracy"]) == set(FORWARD_MAPPING)
    ...
    assert results["three_b_reference"] == THREE_B_REFERENCE
    assert np.asarray(forward["median_similarity_matrix"]).shape == (3, 3)

    lines = headline(results)
    assert len(lines) == 4
    assert all("3B" in line for line in lines), "every number travels with its comparison"
