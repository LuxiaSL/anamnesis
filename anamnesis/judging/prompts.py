...
# Its mode descriptions are paraphrased from the mode definitions themselves — the
# core set of anamnesis/modes/mode_sets.json, with the format constraint dropped —
# which is what keeps them independent of any marker battery that scores the same
# texts. They are banked judge text and stay written out here: a judged number means
# the question that was asked, so this table is pinned rather than derived from a
# registry a reader may extend.
...
MODE = PromptSet(
    ...
    criterion_source=(
        "anamnesis/modes/mode_sets.json, the core set's mode instructions with the "
        "format constraint dropped"
    ),
    answer="json_choice",
    system_template=(
...
VALID_MODES: tuple[str, ...] = mode_set(CORE_MODE_SET).names()
"""The core mode set's labels, in its own label order.

Read from the mode registry rather than restated: this tuple is the order the
rubric's rating keys and the confusion matrix's rows are built in, so a set of
labels that disagreed with the prompts a generation ran under would produce a
judged table keyed to modes the corpus does not have."""
