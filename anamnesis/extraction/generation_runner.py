...
Key design:
  ...
  - Replay manifest written beside the metadata: the realized token ids are known
    here, so a run made by this module is replayable without reconstructing them
    from its decoded text. That is what `replay/manifest.py` has to reconstruct,
    with validation, for a bank that carries text only.
"""
...
def run_single_generation(
    ...
    # The realized [prompt + generated] ids, for the run's replay manifest. Carried
    # on the metadata dict and stripped before metadata reaches disk (save_generation /
    # save_metadata_index): the ids are the manifest's, and two files answering one
    # question is how they drift apart.
    realized_ids = [int(x) for x in outputs.sequences[0].cpu().tolist()]
...
def lean_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    """A generation's metadata as it goes to disk: without the manifest's ids.

    The realized token ids ride the in-memory metadata from the generation to the
    manifest writer and stop there. Both on-disk metadata schemas — the per-gen
    JSON and the run's `metadata.json` — are therefore exactly what every banked
    run already carries.
    """
    return {k: v for k, v in metadata.items() if k != INPUT_IDS_KEY}


def manifest_entry(metadata: dict[str, Any]) -> ReplayEntry:
    """The replay row for a generation this pass just produced.

    Raises
    ------
    KeyError
        When the metadata carries no realized ids, which means it was read back
        from disk rather than produced by `run_single_generation`.
    """
    return entry_from_ids(metadata[INPUT_IDS_KEY], int(metadata["prompt_length"]))
...
def save_metadata_index(
    ...
    with open(metadata_path, "w") as f:
        json.dump({
            ...
            "generations": [lean_metadata(m) for m in all_metadata],
        }, f, indent=2, default=str)


def save_replay_manifest(
    entries: dict[str, ReplayEntry],
    config: ExperimentConfig,
) -> Path:
    """Write the run's replay manifest beside its metadata.

    Every generation this pass produced is an entry; nothing is flagged, because
    the ids are the realized ones rather than a reconstruction. A generation that
    failed is absent from both this file and the metadata's generation list, and
    is named in `failed_ids` there.
    """
    manifest = manifest_from_entries(entries)
    return write_replay_manifest(manifest_path(config.outputs_dir), manifest)
...
def run_experiment(
    ...
    # Replay rows for already-completed generations come from the manifest on disk,
    # not from their re-read metadata: the ids never went into metadata, which is
    # why the manifest is written incrementally alongside it.
    manifest_entries: dict[str, ReplayEntry] = {}
    if completed_ids:
        try:
            manifest_entries = dict(load_replay_manifest(config.outputs_dir).entries)
            logger.info(f"Resumed replay manifest: {len(manifest_entries)} entries")
        except FileNotFoundError:
            logger.warning(
                "Resuming a run with no replay manifest: the generations already on disk "
                "will be absent from it. Rebuild it from the bank with "
                "anamnesis.extraction.replay.manifest once the run finishes."
            )
        except Exception as e:
            logger.warning(f"Could not load the existing replay manifest: {e}")
    ...
    for i, spec in enumerate(tqdm(specs_to_run, desc="Generating")):
        try:
            ...
            manifest_entries[str(spec.generation_id)] = manifest_entry(metadata)
            ...
            if (i + 1) % 25 == 0:
                ...
                save_replay_manifest(manifest_entries, config)
