...
from anamnesis.config import ModelConfig, resolve_preset
...
def default_sampled_layers(config: ModelConfig) -> list[int]:
    """The layer plan a config's preset declares.

    A layer index means a different fraction of the network in every model, so
    there is no layer set that is right for an unnamed one. A config built by
    ``ModelConfig.from_preset()`` carries its preset's name and the plan is read
    from that row; a config assembled by hand carries no name, and the caller has
    to say which layers to hook.

    Raises
    ------
    ValueError
        When the config names no preset, listing what the caller must pass.
    """
    if config.preset_name is None:
        raise ValueError(
            "sampled_layers is unset and this ModelConfig carries no preset name, so the "
            "layer plan cannot be looked up; pass sampled_layers explicitly, or build the "
            "config with ModelConfig.from_preset()"
        )
...
def load_model(
    ...
    Args:
        config: Model configuration, normally `ModelConfig.from_preset(<name>)`.
            Its `attn_implementation` is validated there to be a kernel that
            returns attention weights — extraction without them silently loses
            the attention source, so the constraint lives in the config rather
            than being re-asserted here.
        sampled_layers: Default layer set for gate_proj hooks and (unless
            overridden by key_layers) k_proj hooks. Defaults to the layer plan
            of the config's preset.
